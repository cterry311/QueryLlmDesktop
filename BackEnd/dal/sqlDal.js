...
function getConversationForDisplay(id) {
    ...
    for (const message of messages) {
        ...
        if (message.role === 'assistant' && prev?.role === 'assistant') {
            if (message.content) prev.content += message.content;
        } else {
...
function pushMessage(message, conversationId, modelId) {
    ...
    if (role === 'tool') {
        db.prepare(`
            INSERT INTO tool_results (message_id, tool_call_id, content)
            VALUES (?, ?, ?)
        `).run(messageId, message.tool_call_id, message.content);
    }
...
exports.updateConversationTitle = updateConversationTitle;
exports.getConversationById = getConversationById;
exports.getConversationForDisplay = getConversationForDisplay;